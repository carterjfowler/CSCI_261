...
bool Date::earlierDate(Date &that) {
   //Should compare values to see which is greater
   //Check year first, then month, then date
   if(_year > that.getYear()) {
      return false;
   } else if (_year == that.getYear()) {
      if (_month > that.getMonth()) {
         return false;
      } else if (_month == that.getMonth()) {
         if (_date > that.getDay()) {
            return false;
         } else if (_date == that.getDay()) {
            return false;
         } else {
            return true;
         }
